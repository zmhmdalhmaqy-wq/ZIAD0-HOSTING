...
# ============== تثبيت المكتبات ==============
def install_requirements(server_path, log_file):
    """تثبيت المكتبات من requirements.txt تلقائياً"""
    req_file = os.path.join(server_path, "requirements.txt")
    if not os.path.exists(req_file):
        return True
    
    try:
        log_file.write(f"\n{'='*50}\n📦 جاري تثبيت المكتبات من requirements.txt...\n{'='*50}\n")
        log_file.flush()
        ...
        log_file.write(f"📋 المكتبات المطلوبة: {', '.join(packages)}\n")
        log_file.flush()
        ...
        # قراءة المخرجات وكتابتها في السجل
        ...
        if proc.returncode == 0:
            ...
            log_file.write(f"\n⚠️ فشل تثبيت بعض المكتبات (كود: {proc.returncode})\n{'='*50}\n\n")
            log_file.flush()
            return False
            
    except Exception as e:
        log_file.write(f"\n❌ خطأ في تثبيت المكتبات: {str(e)}\n\n")
        log_file.flush()
        return False

# ============== تشغيل السيرفرات ==============
def start_server_process(folder):
    """تشغيل السيرفر مع تثبيت تلقائي للمكتبات"""
    ...
    # البحث عن ملف التشغيل إذا لم يكن محدداً
    if not main_file:
        for f in ["main.py", "app.py", "bot.py", "index.py", "server.py", "run.py"]:
    ...
    install_requirements(srv["path"], log_file)
    
    # تشغيل السيرفر
    try:
        # إضافة مسار السيرفر إلى PYTHONPATH
        env = os.environ.copy()
        env["PORT"] = str(port)
        env["PYTHONPATH"] = srv["path"] + os.pathsep + env.get("PYTHONPATH", "")
        ...
        log_file.write(f"\n✅ السيرفر يعمل الآن (PID: {proc.pid})\n{'='*60}\n\n")
        log_file.flush()
        return True
...
def restart_server(folder):
    """إعادة تشغيل السيرفر عند التوقف"""
    srv = db["servers"].get(folder)
    if not srv:
        return
    ...
    # انتظار قليلاً قبل إعادة التشغيل
    time.sleep(2)
    
    # محاولة التشغيل مرة أخرى
    if start_server_process(folder):
        print(f"✅ تم إعادة تشغيل السيرفر {folder} تلقائياً")
        return True
    else:
        print(f"❌ فشل إعادة تشغيل السيرفر {folder}")
        return False

# ============== مراقبة العمليات ==============
def process_monitor():
    """مراقبة السيرفرات وإعادة تشغيلها إذا توقفت"""
    while True:
        try:
                ...
                if srv.get("status") == "Running" and srv.get("pid"):
                    try:
                        ...
                        if not p.is_running() or p.status() == psutil.STATUS_ZOMBIE:
                            print(f"⚠️ السيرفر {folder} توقف، جاري إعادة التشغيل...")
                            restart_server(folder)
                    except psutil.NoSuchProcess:
                        print(f"⚠️ السيرفر {folder} غير موجود، جاري إعادة التشغيل...")
                        restart_server(folder)
                    except:
                        pass
        except:
...
def list_servers():
    ...
    user = db["users"].get(session["username"], {"max_servers": 999999, "expiry_days": 365})
    max_srv = user.get("max_servers", 999999)
...
def server_action(folder, action):
    ...
    if action == "start":
        ...
        time.sleep(1)
        if start_server_process(folder):
            return jsonify({"success": True, "message": "تم إعادة تشغيل السيرفر"})
        else:
            return jsonify({"success": False, "message": "فشل إعادة التشغيل"})
        ...
        save_db(db)
        return jsonify({"success": True, "message": "تم حذف السيرفر"})
...
@app.route('/api/files/download/<folder>/<filename>')
...
def rename_user_file(folder):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "تم تعديل الاسم"})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)})
...
def install_requirements_api(folder):
    """تثبيت المكتبات يدوياً من requirements.txt"""
    if "username" not in session:
        return jsonify({"success": False}), 401
    srv = db["servers"].get(folder)
    if not srv or srv["owner"] != session["username"]:
    ...
    with open(log_path, "a", encoding='utf-8') as log_file:
        success = install_requirements(srv["path"], log_file)
    
    if success:
        return jsonify({"success": True, "message": "تم تثبيت المكتبات بنجاح"})
    else:
        return jsonify({"success": False, "message": "فشل تثبيت بعض المكتبات، راجع السجل"})
...
def admin_users():
    ...
    for uname, udata in db["users"].items():
        users_list.append({
            ...
            "max_servers": udata.get("max_servers", 999999),
            "expiry_days": udata.get("expiry_days", 365),
            "password_preview": "••••••••"
        })
    return jsonify({"success": True, "users": users_list})

@app.route('/api/admin/get-password')
def admin_get_password():
    ...
    if username == ADMIN_USERNAME:
        return jsonify({"success": True, "password": ADMIN_PASSWORD_RAW})
    
    return jsonify({"success": False, "message": "لا يمكن عرض كلمة المرور"})
...
def admin_toggle_ban():
    ...
    db["users"][username]["is_banned"] = ban
    save_db(db)
    
    return jsonify({"success": True, "message": f"تم {'حظر' if ban else 'فك حظر'} المستخدم"})
...
def admin_create_user():
    ...
    username = data.get("username", "").strip()
    password = data.get("password", "").strip()
    max_servers = int(data.get("max_servers", 999999))
...
def admin_stats():
    ...
    total_servers = len(db["servers"])
    banned_users = len([u for u in db["users"].values() if u.get("is_banned", False)])
...
@app.route('/api/admin/user-files')
def admin_user_files():
    ...
    user_dir = os.path.join(USERS_DIR, target_user)
...
def admin_rename_file():
    ...
    if not target_user or not old_name or not new_name:
    ...
    if not os.path.exists(old_path):
        return jsonify({"success": False, "message": "الملف الأصلي غير موجود"})
